...
public class MahoutController {
	...
	public String index(Locale locale, ModelMap model,HttpServletRequest request) {
		...
		String writeFilePath=cl.getResource("movies/ml-1m/ratings.csv").getFile();
	...
	public String recommender(Locale locale, ModelMap model,HttpServletRequest request) {
		...
		try {
			...
			{
				neighborhood=new NearestNUserNeighborhood(Integer.parseInt(nums), similarity, dataModel);
			}
			else
			{
			...
			List<RecommendedItem> recommendations = recommender.recommend(Long.parseLong(userId), Integer.parseInt(nums));
